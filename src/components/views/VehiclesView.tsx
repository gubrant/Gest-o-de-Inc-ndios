  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black tracking-tighter text-white uppercase italic">Controle de Frota</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[#1A1A1A] border border-white/5 p-5 rounded-xl">
          ...
          <p className="text-3xl font-bold text-white tracking-tighter">{vehicles.length}</p>
        </div>
        <div className="bg-[#1A1A1A] border border-white/5 p-5 rounded-xl border-l-4 border-l-green-500/50">
          <p className="text-green-500 text-[10px] font-bold uppercase tracking-[0.2em] mb-1">Em Operação</p>
          <p className="text-3xl font-bold text-white tracking-tighter">{vehicles.filter(v => v.status === 'active').length}</p>
        </div>
        <div className="bg-[#1A1A1A] border border-white/5 p-5 rounded-xl border-l-4 border-l-yellow-500/50">
          <p className="text-yellow-500 text-[10px] font-bold uppercase tracking-[0.2em] mb-1">Manutenção</p>
          <p className="text-3xl font-bold text-white tracking-tighter">{vehicles.filter(v => v.status === 'maintenance').length}</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#0A0A0A]">
              <tr>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-white/5">Placa/ID</th>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-white/5">Patrimônio</th>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-white/5">Tipo / Modelo</th>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-white/5">Fotos</th>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-white/5 text-right">Ação</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {filteredVehicles.map(vehicle => (
                <tr key={vehicle.id} className="hover:bg-white/[0.02] transition-colors group">
                  <td className="p-4 text-sm font-mono font-bold text-white uppercase tracking-tighter">{vehicle.plate}</td>
                  <td className="p-4">
                    {vehicle.patrimony ? (
                  ...
                  <td className="p-4">
                    <div className="flex flex-col">
                      <span className="text-sm text-slate-400 font-medium uppercase tracking-tight italic leading-none mb-1">{vehicle.type}</span>
                      <span className="text-[8px] font-black uppercase tracking-wider text-slate-600 italic">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex justify-end gap-2">
                       <button 
                         ...
                         className="p-1.5 hover:bg-white/5 text-slate-500 hover:text-white rounded transition-all"
                         title="Editar Veículo"
                       >
                         <Edit2 size={16} />
                       </button>
                       <button onClick={() => updateStatus(vehicle, 'active')} className="p-1.5 hover:bg-green-500/10 text-slate-600 hover:text-green-500 rounded transition-all"><CheckCircle2 size={16} /></button>
                       <button onClick={() => updateStatus(vehicle, 'maintenance')} className="p-1.5 hover:bg-yellow-500/10 text-slate-600 hover:text-yellow-500 rounded transition-all"><Wrench size={16} /></button>
                       <button onClick={() => updateStatus(vehicle, 'left')} className="p-1.5 hover:bg-red-500/10 text-slate-600 hover:text-red-500 rounded transition-all"><XCircle size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          ...
          >
            <div className="flex items-center gap-3 mb-6 border-b border-white/5 pb-4">
               ...
               <div>
                  <h3 className="text-xl font-bold text-white uppercase tracking-tighter italic">{editingId ? 'Editar Veículo' : 'Novo Veículo'}</h3>
