...
export default function MapView({ incidentId }: MapViewProps) {
  ...
  return (
    <div className="h-full flex flex-col relative">
      <header className="flex items-center justify-between mb-4 bg-[#121212] p-4 rounded-xl border border-white/5 no-print">
        <div className="flex items-center gap-4">
          <div className="bg-orange-600/20 p-2 rounded-lg">
            <Layers className="text-orange-500" size={20} />
          </div>
          <div>
            <h2 className="text-lg font-black text-white italic uppercase tracking-tighter">SIG Operacional</h2>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Sistema de Informações Geográficas</p>
          </div>
        </div>
        
        <button 
          onClick={() => setShowAddModal(true)}
          className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2"
        >
          <Plus size={14} />
          Importar KML
        </button>
      ...
      <div className="flex-1 rounded-2xl overflow-hidden border border-white/5 bg-[#1A1A1A] relative shadow-2xl">
        <MapContainer 
        ...
        <div className="absolute left-4 top-4 z-10 w-48 no-print">
          <div className="bg-[#121212]/95 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-2xl">
            <div className="p-3 border-b border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Camadas Ativas</span>
              <span className="bg-orange-500 text-white text-[9px] px-1.5 py-0.5 rounded font-black">{layers.length}</span>
            </div>
            <div className="max-h-48 overflow-y-auto custom-scrollbar p-1">
              {layers.length === 0 ? (
                <p className="p-3 text-[9px] text-slate-600 italic uppercase">Sem camadas</p>
              ) : (
                layers.map(layer => (
                  <div key={layer.id} className="flex items-center justify-between p-2 hover:bg-white/5 rounded group transition-colors">
                    <span className="text-[10px] font-bold text-slate-300 truncate w-32 uppercase italic">{layer.name}</span>
                    <button 
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className={cn(
                      "flex-1 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border flex items-center justify-center gap-2",
                      uploadType === 'file' ? "bg-orange-600 border-orange-600 text-white" : "bg-white/5 border-white/10 text-slate-500"
                    )}
                  >
                    <Upload size={14} />
                    Arquivo .KML
                  ...
                  <button 
                    ...
                    className={cn(
                      "flex-1 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border flex items-center justify-center gap-2",
                      uploadType === 'text' ? "bg-orange-600 border-orange-600 text-white" : "bg-white/5 border-white/10 text-slate-500"
                    )}
                  >
                    <FileCode size={14} />
                    Código KML
                ...
                {uploadType === 'file' ? (
                  <div 
                    ...
                    className="border-2 border-dashed border-white/5 rounded-2xl p-8 text-center cursor-pointer hover:border-orange-600/30 hover:bg-orange-600/5 transition-all group"
                  >
                    <Upload className="mx-auto text-slate-600 mb-3 group-hover:text-orange-500 transition-colors" size={32} />
                  ...
                  <div>
                    ...
                    <textarea 
                      className="w-full bg-[#1A1A1A] border-white/5 border rounded-lg text-[10px] p-3 outline-none text-slate-400 font-mono transition-all focus:border-orange-600 min-h-[150px]"
                ...
                <div className="pt-4">
                  <button 
                    ...
                    className="w-full bg-orange-600 hover:bg-orange-700 disabled:opacity-50 disabled:bg-slate-800 text-white py-4 rounded-xl font-black uppercase text-xs tracking-widest transition-all shadow-[0_0_20px_rgba(234,88,12,0.3)]"
                  >
