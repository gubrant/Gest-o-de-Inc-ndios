  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black tracking-tighter text-slate-900 uppercase italic">Logística de Alimentação</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {Object.entries(mealLabels).map(([key, label]) => {
          ...
          return (
            <div key={key} className="bg-white border border-slate-200 p-5 rounded-xl flex items-center gap-4 group hover:border-orange-500/20 transition-all shadow-sm">
              <div className="bg-orange-600/10 p-3 rounded-lg text-orange-600 transition-transform group-hover:scale-110">
                {mealIcons[key as MealType]}
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">{label} (HOJE)</p>
                <p className="text-3xl font-black text-slate-900 tracking-tighter">{totalForMeal}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-lg">
        <div className="p-4 border-b border-slate-100 bg-slate-50">
           <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-widest italic">Histórico de Movimentação</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50">
              <tr>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-slate-100">Data</th>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-slate-100">Refeição</th>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-slate-100">Quant.</th>
                <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-slate-100">Observações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {demands.map(demand => (
                <tr key={demand.id} className="hover:bg-slate-50 transition-colors group">
                  <td className="p-4 text-xs font-mono font-bold text-slate-900 italic tracking-tighter">{demand.date}</td>
                  <td className="p-4">
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-orange-600 italic">
                      <span className="p-1 bg-orange-600/10 rounded">{mealIcons[demand.mealType]}</span>
                      {mealLabels[demand.mealType]}
                    </div>
                  </td>
                  <td className="p-4 text-sm font-black text-slate-900 uppercase tracking-tight">{demand.count} <span className="text-[9px] text-slate-400">PORÇÕES</span></td>
                  <td className="p-4 text-[10px] text-slate-400 font-bold uppercase tracking-widest italic">{demand.notes || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <motion.div 
            ...
            className="bg-white border border-slate-200 p-8 rounded-2xl w-full max-w-sm shadow-2xl"
          >
            <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
               <div className="p-2 bg-orange-600/10 rounded-lg text-orange-600"><UtensilsCrossed size={20}/></div>
               <div>
                  <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tighter italic">Nova Demanda</h3>
