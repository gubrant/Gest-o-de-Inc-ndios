...
export default function OperationPhotosView({ incidentId }: OperationPhotosViewProps) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center justify-between mb-8 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden no-print">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-900 italic uppercase tracking-tighter leading-none">Fotos de Campo</h2>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Registros Fotográficos da Operação ({photos.length}/50)</p>
          </div>
        </div>
        ...
        <button 
          ...
          className="bg-slate-900 hover:bg-black disabled:opacity-50 disabled:bg-slate-100 text-white px-6 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2 shadow-xl shadow-slate-200"
        >
          {uploading ? (
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {loading ? (
          ...
          <div className="bg-white border border-dashed border-slate-200 rounded-3xl p-20 text-center">
            ...
            <h3 className="text-slate-900 font-black italic uppercase tracking-tighter text-xl mb-2">Sem Registros Visuais</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {photos.map((photo) => (
              <motion.div 
                ...
                className="group relative aspect-square bg-slate-50 rounded-2xl overflow-hidden border border-slate-200 cursor-pointer hover:border-orange-600/30 transition-all shadow-md"
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
                   <div className="flex items-center justify-between">
                     <span className="text-[8px] font-mono text-slate-200 font-bold uppercase tracking-widest">
      ...
      <AnimatePresence>
        {selectedPhoto && (
          <div 
            className="fixed inset-0 bg-slate-900/95 z-[100] p-4 flex items-center justify-center backdrop-blur-xl"
          ...
          >
            <motion.div 
              ...
              className="relative max-w-[90vw] max-h-[90vh] rounded-3xl overflow-hidden border border-white/10 shadow-2xl bg-black"
            >
